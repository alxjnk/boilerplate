...
export const getLastMessageCreateDate = (messages, user) =>
	...
	new Date() - new Date([...messages[user]][0].createdAt)
...
export const getNumberOfNewMessages = (messages, user) => {
	const numberOfNewMessages = messages[user]
		.map(item => (item.viewed ? 0 : 1))
		.reduce((result, num) => result + num, 0)
		? messages[user].map(item => (item.viewed ? 0 : 1)).reduce((result, num) => result + num, 0)
		: '';
...
// сортируем массив сообщений по user_id
export const sortMessages = messages => {
	const sortedMessages = {};
	const tempSortedMessages = {};
	const users_id = getFieldValues(messages, 'user_id');
	...
	if (Object.keys(tempSortedMessages)) {
		Object.keys(tempSortedMessages)
			.sort(
				(a, b) =>
					new Date(tempSortedMessages[b][0].createdAt) * 1 - new Date(tempSortedMessages[a][0].createdAt) * 1,
			)
			.forEach(field => {
...
export const filterBookingsData = (bookingsData, startDate, endDate) => {
	...
	rooms.forEach(room => {
		bookingsData.forEach(booking => {
			...
			if (
				...
				((new Date(newBooking.arrival) * 1 <= new Date(format(new Date(startDate), 'RRRR-LL-dd')) * 1 &&
					...
					(new Date(newBooking.arrival) * 1 >= new Date(format(new Date(startDate), 'RRRR-LL-dd')) * 1 &&
						new Date(newBooking.arrival) * 1 <=
							new Date(format(new Date(endDate), 'RRRR-LL-dd')) * 1 + 86399999) ||
					(new Date(newBooking.departure) * 1 >= new Date(format(new Date(startDate), 'RRRR-LL-dd')) * 1 &&
						new Date(newBooking.departure) * 1 <=
							new Date(format(new Date(endDate), 'RRRR-LL-dd')) * 1 + 86399999))
			) {
				...
				if (new Date(newBooking.arrival) * 1 <= startDate) {
					...
					newBooking.start = Math.ceil(
						(new Date(newBooking.arrival) * 1) / 24 / 60 / 60 / 1000 - startDate / 24 / 60 / 60 / 1000,
					);
					newBooking.startDay = new Date(newBooking.arrival).getDate();
				}
				if (new Date(newBooking.departure) * 1 >= endDate) {
					newBooking.end = Math.ceil(
						new Date(endDate / 24 / 60 / 60 / 1000 - startDate / 24 / 60 / 60 / 1000),
					);
					newBooking.endDay = new Date(newBooking.departure).getDate();
				} else {
					newBooking.end = Math.ceil(
						(new Date(newBooking.departure) * 1) / 24 / 60 / 60 / 1000 - startDate / 24 / 60 / 60 / 1000,
					);
					newBooking.endDay = new Date(newBooking.departure).getDate();
				}
		...
		if (sortedBookingsData[room]) {
			sortedBookingsData[room].sort((a, b) => new Date(b.arrival) * 1 - new Date(a.arrival) * 1);
		}
	});
...
export const checkInBookings = (bookings, date, today) => {
	...
	checkInBookings = checkInBookings.filter(item => item.arrival >= today && item.arrival <= date);
...
export const checkOutBookings = (bookings, date, today) => {
	...
	checkOutBookings = checkOutBookings.filter(item => item.departure >= today && item.departure <= date);
...
export const checkInBookingsSorter = bookings => {
	...
	sortedBookings.sort((a, b) => new Date(a.arrival) * 1 - new Date(b.arrival) * 1);
...
export const checkOutBookingsSorter = bookings => {
	...
	sortedBookings.sort((a, b) => new Date(a.departure) * 1 - new Date(b.departure) * 1);
